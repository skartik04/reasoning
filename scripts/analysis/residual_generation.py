#!/usr/bin/env python3
"""
Record residuals during actual greedy generation for harmful questions.
...
def load_harmful_questions(dataset_path, n_samples=None):
    """Load questions from qa_harmful.json dataset"""
    print(f"Loading harmful questions from: {dataset_path}")
    ...
    print(f"Loaded {len(questions)} harmful questions")
...
def record_generation_residuals(model, tokenizer, questions, n_generate_tokens=10, max_questions=100):
    ...
    residuals_data = {
        ...
        'metadata': {
            ...
            'description': 'Residuals recorded during actual greedy generation using chat template formatting'
        }
    }
...
def save_residuals(residuals_data, model_name, output_folder="residuals"):
    ...
    # Save residuals
    filename = f"harmful_generation_residuals.pt"
...
def main():
    ...
    dataset_path = "artifacts/data/qa_harmful.json"
    n_questions = 200  # Number of questions to process (testing with small number first)
    n_generate_tokens = 40  # Number of tokens to generate per question (testing with small number first)
    output_folder = "artifacts/residuals"
    
    print("=" * 80)
    print("RESIDUAL RECORDING DURING GREEDY GENERATION")
    print("=" * 80)
    ...
    # Load questions
    questions = load_harmful_questions(dataset_path, n_samples=n_questions)
