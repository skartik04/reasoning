...
def load_harmful_dataset(dataset_path="artifacts/data/splits/harmful_test.json"):
...
def save_progress_and_exit(questions_data, inference_file, error_msg):
    ...
    if questions_data:
        output_dir = os.path.dirname(inference_file)
        if output_dir:
...
def run_complete_experiment(harmful_questions, model, tokenizer, device, client, model_name, 
    ...
    results_dir = os.path.join("artifacts", "results", "analysis")
    os.makedirs(results_dir, exist_ok=True)
    inference_file = os.path.join(results_dir, f'harmful_inference_{model_filename}_offset{offset}_{n}.json')
    ...
    complete_file = os.path.join(results_dir, f'harmful_analysis_{model_filename}_offset{offset}_{n}.json')
...
if __name__ == "__main__":
    ...
    offset = 120
    n = 400
    dataset_path = "artifacts/data/splits/harmful_test.json"
